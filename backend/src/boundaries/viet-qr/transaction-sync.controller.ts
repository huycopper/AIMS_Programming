...
} from '../../pay-order/pay-by-vietqr/boundary/webhook/dto/vietqr-transaction-sync.response.js';
  ...
  async transactionSync(
    ...
    try {
      ...
      const transactionRef =
        transactionSyncBody.referencenumber ||
        transactionSyncBody.transactionid; // Mã do VietQR tạo

      // Bước 4: Tạo bản ghi PaymentTransaction và lưu vào database
      const paymentTransaction = this.buildPaymentTransaction(
        order,
        transactionSyncBody,
        transactionRef,
        refTransactionId,
