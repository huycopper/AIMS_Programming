...
export class PayOrderController {
  ...
  ): Promise<GenerateVietQrPaymentResponse> {
  ...
  async confirmPayment(
    ...
    const paymentResult =
      await this.payThroughVietQRController.confirmPayment(order);
  ...
  async getPaymentConfirmation(
    ...
    this.logger.log(
      `Received payment confirmation status request for order: ${orderId}`,
    );
